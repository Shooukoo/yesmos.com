...
export async function GET() {
    try {
        ...
        const response = await fetch(url, {
            ...
            // Importante: no-store evita el error de "Next.js data cache over 2MB"
            cache: 'no-store'
        });
        ...
        $(".fly").each((index, element) => {
            ...
            // CORRECCIÓN DE IMAGEN: Convertir ruta relativa a absoluta
            let image = $(element).find(".imgProd img").attr("src") || "";
            if (image && !image.startsWith('http')) {
                // Limpiamos la ruta y añadimos el dominio base
            ...
            if (name !== "Sin nombre" && price > 0) {
                products.push({
                    ...
                    price,
                    category,
                    image, // Ahora contiene la URL completa
                    url: productUrl,
                    available: true
                });
            }
        ...
        console.error("Error en scraping local:", error);
        return NextResponse.json({ error: "Error cargando catálogo local" }, { status: 500 });
    }
}